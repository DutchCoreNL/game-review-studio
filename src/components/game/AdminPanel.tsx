...
import { Shield, Trash2, RotateCcw, Ban, RefreshCw, AlertTriangle, Filter, MessageCircleWarning, VolumeX, X, History } from 'lucide-react';
  ...
  const fetchSanctions = async (entry: LeaderboardEntry) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('admin-actions', {
        body: { action: 'get_sanctions', userId: entry.user_id },
      });
      if (error) throw error;
  ...
  const revokeSanction = async (sanctionId: string) => {
    try {
      const { error } = await supabase.functions.invoke('admin-actions', {
        body: { action: 'revoke_sanction', entryId: sanctionId },
      });
      if (error) throw error;
      ...
      {historyPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="game-card w-full max-w-xs space-y-3 max-h-[70vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold flex items-center gap-1.5">
                <History size={12} /> Sancties: {historyPopup.entry.username}
              </h3>
              ...
              <div className="space-y-1.5">
                {historyPopup.sanctions.map(s => (
                  <div key={s.id} className={`p-2 rounded border text-[0.5rem] space-y-0.5 ${s.active ? 'border-gold/30 bg-gold/5' : 'border-border bg-muted/50 opacity-60'}`}>
                    <div className="flex items-center justify-between">
                      <GameBadge variant={s.type === 'warning' ? 'gold' : 'ice'} size="xs">
                        {s.type === 'warning' ? '⚠️ Waarschuwing' : '🔇 Mute'}
                      </GameBadge>
                      {s.active && (
                        <button onClick={() => revokeSanction(s.id)} className="text-[0.45rem] text-blood hover:underline">Intrekken</button>
                      )}
                      {!s.active && <span className="text-[0.45rem] text-muted-foreground">Inactief</span>}
                    </div>
