...
import { SCRAP_VALUES, CRAFT_RECIPES, getWeaponScrapValue, getGearScrapValue } from '@/game/salvage';
import { GEAR_RARITY_COLORS, GEAR_RARITY_LABEL } from '@/game/gearGenerator';
...
export function SalvageView() {
  ...
  const rarityLabel = (r: string) => GEAR_RARITY_LABEL[r as keyof typeof GEAR_RARITY_LABEL] || r;
  ...
  return (
    <div className="space-y-4 p-2 sm:p-4 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h2 className="text-lg font-display font-bold text-foreground tracking-wide">SALVAGE & CRAFTING</h2>
      ...
      <div className="flex gap-2">
        <Button variant={tab === 'salvage' ? 'default' : 'outline'} size="sm" onClick={() => setTab('salvage')}>
          <Hammer size={14} /> Ontmantelen
        </Button>
        <Button variant={tab === 'craft' ? 'default' : 'outline'} size="sm" onClick={() => setTab('craft')}>
          <Wrench size={14} /> Crafting
        </Button>
      </div>
            ...
            {showWeapons && (
              <div className="grid gap-1.5 mt-1">
                ...
                {salvageableWeapons.map(w => (
                  <div key={w.id} className="flex items-center justify-between border border-border rounded px-3 py-2 bg-muted/10">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold truncate text-foreground">{w.name}</p>
                      <span className={`text-[0.5rem] font-bold ${rarityColor(w.rarity)}`}>{rarityLabel(w.rarity)}</span>
                    </div>
                    <Button size="sm" variant="destructive" onClick={() => handleSalvageWeapon(w.id)}>
            ...
            {showArmor && (
              <div className="grid gap-1.5 mt-1">
                ...
                {salvageableArmor.map(g => (
                  <div key={g.id} className="flex items-center justify-between border border-border rounded px-3 py-2 bg-muted/10">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold truncate text-foreground">{g.name}</p>
                      <span className={`text-[0.5rem] font-bold ${rarityColor(g.rarity)}`}>{rarityLabel(g.rarity)}</span>
                    </div>
                    <Button size="sm" variant="destructive" onClick={() => handleSalvageGear(g.id, 'armor')}>
              ...
              <div className="grid gap-1.5 mt-1">
                ...
                {salvageableGadgets.map(g => (
                  <div key={g.id} className="flex items-center justify-between border border-border rounded px-3 py-2 bg-muted/10">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleSalvageGear(g.id, 'gadget')}>
                      +{getGearScrapValue(g)} scrap
                    </Button>
                  </div>
                ))}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" disabled={!canAfford} onClick={() => handleCraft(recipe.id)}>
