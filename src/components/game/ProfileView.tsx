...
import { Swords, Brain, Gem, Sword, Shield, Smartphone, Trophy } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div className="game-card border-l-[3px] border-l-gold mb-4">
        ...
        <div className="text-right text-[0.55rem] text-muted-foreground mt-1">
          XP: {state.player.xp} / {state.player.nextXp}
        </div>
      </div>
        ...
        {STAT_INFO.map(s => {
          ...
          return (
            <div key={s.id} className="flex items-center gap-2 text-xs">
              <div className="w-16 flex items-center gap-1.5 text-muted-foreground">
                {s.icon}
                <span>{s.label}</span>
              </div>
              ...
              {state.player.skillPoints > 0 && (
                <button
                  ...
                  className="w-5 h-5 rounded bg-muted border border-gold text-gold text-xs flex items-center justify-center hover:bg-[hsl(var(--gold)/0.1)]"
                >
                  +
                </button>
              )}
      ...
      <div className="space-y-2 mb-4">
        {state.ownedGear
          .filter(id => !Object.values(state.player.loadout).includes(id))
          .map(id => {
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {ACHIEVEMENTS.map(a => {
          ...
          return (
            <div
              key={a.id}
              className={`game-card flex items-center gap-2 ${unlocked ? 'border-gold' : 'opacity-40'}`}
            >
              <Trophy size={14} className={unlocked ? 'text-gold' : 'text-muted-foreground'} />
              <div>
                <div className="text-[0.6rem] font-bold">{a.name}</div>
                <div className="text-[0.5rem] text-muted-foreground">{a.desc}</div>
              </div>
            </div>
          );
        })}
          ...
          <div className="game-card border-l-[3px] border-l-blood mb-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[0.6rem] text-muted-foreground">3% rente per dag</p>
              </div>
              <button
                onClick={() => {
                  const amt = Math.min(5000, state.money, state.debt);
                  dispatch({ type: 'PAY_DEBT', amount: amt });
                  showToast(`€${amt.toLocaleString()} afgelost`);
                }}
                className="px-3 py-1.5 rounded text-[0.65rem] font-bold bg-[hsl(var(--blood)/0.1)] border border-blood text-blood"
              >
                AFLOSSEN
              </button>
            </div>
