...
// Personality labels are now in i18n
...
export function OperationsView() {
  ...
  const handleAcceptContract = async () => {
    ...
    try {
      ...
      showToast(t.ops.connectionError, true);
    } finally {
  ...
  const tabs: SubTab<string>[] = [
    ...
    { id: 'contracts', label: t.ops.contract, icon: <Swords size={12} />, badge: state.activeContracts.length },
    { id: 'heists', label: t.ops.heist, icon: <Target size={12} /> },
    { id: 'hits', label: t.ops.hits, icon: <Skull size={12} />, badge: (state.hitContracts || []).filter(h => h.deadline >= state.day).length },
    { id: 'bounties', label: t.ops.bounties, icon: <ShieldAlert size={12} />, badge: (state.activeBounties || []).length + (state.placedBounties || []).length },
    { id: 'wanted', label: t.ops.wanted, icon: <Target size={12} /> },
    { id: 'crew', label: t.ops.crew, icon: <Users size={12} />, badge: state.crew.length },
    { id: 'challenges', label: t.ops.goals, icon: <Target size={12} />, badge: state.dailyChallenges?.filter(c => c.completed && !c.claimed).length || 0 },
    { id: 'pvp', label: t.ops.pvp, icon: <Swords size={12} /> },
  ];
  ...
  return (
    <ViewWrapper bg={operationsBg}>
      ...
      {subTab === 'solo' && (
        <>
          <SectionHeader title={t.ops.soloTitle} icon={<Crosshair size={12} />} />
          <p className="text-[0.55rem] text-muted-foreground mb-3">{t.ops.soloDesc}</p>
      ...
      {subTab === 'contracts' && (
        <>
          <SectionHeader title={t.ops.contractTitle} icon={<Swords size={12} />} badge={`${state.activeContracts.length}/5`} badgeColor="blood" />
          <p className="text-[0.55rem] text-muted-foreground mb-3">{t.ops.contractDesc}</p>
          ...
          {state.activeContracts.length < 5 && (
            ...
            >
              ...
              {acceptingContract ? t.ops.searching : t.ops.searchContract}
            </GameButton>
          )}

          {state.activeContracts.length === 0 ? (
            <div className="game-card text-center py-6">
              <p className="text-xs text-muted-foreground italic">{t.ops.noContracts}</p>
            </div>
          ) : (
      ...
      {subTab === 'crew' && (
        <>
          <SectionHeader title={t.ops.crewTitle} icon={<Users size={12} />} badge={`${state.crew.length}/6`} />
          ...
          <div className="space-y-2 mb-3">
              ...
              <div key={i} className="game-card">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-bold text-xs">
                        ...
                        {state.crewPersonalities?.[i] && (
                          <span className="ml-1 text-[0.45rem] font-semibold text-ice bg-ice/10 px-1.5 py-0.5 rounded border border-ice/20">
                            {t.ops.personality[state.crewPersonalities[i]] || state.crewPersonalities[i]}
                          </span>
                        )}
                      </h4>
                      {/* Specialization info */}
                        ...
                        <div className="mt-0.5">
                          {[3, 5, 7, 9].some(lvl => c.level >= lvl) && c.level < 10 ? (
                            ...
                            <span className="text-[0.45rem] text-muted-foreground italic">
                              {t.ops.specUnlock} {[3, 5, 7, 9].find(l => l > c.level) || 3}
                            </span>
                          )}
                        </div>
                      )}
                      ...
                      {(() => {
                        ...
                        const loyaltyLabel = loyalty >= 80 ? t.ops.loyal : loyalty >= 50 ? t.ops.neutral : loyalty >= 20 ? t.ops.restless : t.ops.disloyal;
                  ...
                  <div className="flex flex-col items-end gap-1">
                    {c.hp <= 0 ? (
                      <GameBadge variant="blood" size="xs">{t.ops.unconscious}</GameBadge>
                    ) : c.hp < 100 ? (
                      <button
                        onClick={() => {
                          dispatch({ type: 'HEAL_CREW', crewIndex: i });
                          const cost = (100 - c.hp) * costPerHp;
                           if (state.money >= cost) showToast(`${c.name} ${t.ops.healed}`);
                           else showToast(t.common.notEnoughMoney, true);
                        }}
                      ...
                      <GameBadge variant="emerald" size="xs">{t.ops.ready}</GameBadge>
                    )}
            ...
            {state.crew.length === 0 && (
              <div className="game-card text-center py-6">
                ...
                <p className="text-muted-foreground text-xs">{t.ops.noCrew}</p>
              </div>
            )}
          </div>
          ...
          <GameButton variant="gold" fullWidth
            ...
            onClick={() => {
              dispatch({ type: 'RECRUIT' });
               if (state.crew.length < 6 && state.money >= 2500) showToast(t.ops.newCrewMember);
               else if (state.crew.length >= 6) showToast(t.ops.crewFull, true);
               else showToast(t.common.notEnoughMoney, true);
             }}>
             {t.ops.hireSpecialist} (€2.500)
          </GameButton>
        </>
      )}
      ...
      <ConfirmDialog
        open={fireConfirm !== null}
         title={t.ops.fireCrewConfirm}
         message={fireConfirm !== null && state.crew[fireConfirm] ? t.ops.fireConfirmMsg.replace('{name}', state.crew[fireConfirm].name) : ''}
        confirmText={t.ops.fireBtn}
        variant="danger"
        onConfirm={() => {
          if (fireConfirm !== null) {
            const name = state.crew[fireConfirm]?.name;
            dispatch({ type: 'FIRE_CREW', crewIndex: fireConfirm });
            showToast(`${name} ${t.ops.fired}`);
          }
          setFireConfirm(null);
        }}
        ...
        <AnimatePresence>
          {isExpanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mt-2 pt-2 border-t border-border/30">
              ...
              {crew.length === 0 ? (
                <p className="text-[0.55rem] text-muted-foreground italic">{t.ops.noCrewAvailable}</p>
              ) : (
                    ...
                    return (
                      <button key={i} onClick={() => canAssign && onAssign(i)} disabled={!canAssign}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isIdeal && <GameBadge variant="gold" size="xs">{t.ops.ideal}</GameBadge>}
                        </div>
                        <div className="flex items-center gap-1">
                          <Heart size={8} className={c.hp < 30 ? 'text-blood' : 'text-emerald'} />
               ...
               <button onClick={onDrop} className="w-full mt-2 py-1.5 px-2 rounded-lg text-[0.5rem] font-bold text-blood border border-blood/30 hover:bg-blood/10 transition-colors flex items-center justify-center gap-1">
                 <Trash2 size={10} /> {t.ops.cancelContract}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
